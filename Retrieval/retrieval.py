...
def build_query_analysis(query_text: str) -> dict:
    ...
    # Instrument detection: require terms that genuinely signal measurement/psychometric intent.
    # Generic words like "tool" or "tools" alone are too broad — they appear in "digital health tools"
    # which is not a psychometric query. Require at least one specific measurement term.
    _instrument_specific = {"instrument", "questionnaire", "scale", "measure", "measurement",
                             "survey", "psychometric", "validation", "inventory", "checklist"}
    _instrument_broad = {"tool", "tools", "assessment", "assess"}
    _instrument_matches_specific = [t for t in _instrument_specific if t in normalized_query]
    _instrument_matches_broad = [t for t in _instrument_broad if re.search(rf"(?<![a-z]){re.escape(t)}(?![a-z])", normalized_query)]
    if _instrument_matches_specific or (len(_instrument_matches_broad) >= 2):
        detected_query_type = "instrument"
    elif any(term in normalized_query for term in ("qualitative", "interview", "focus group", "perception", "experience",
             "acceptability", "feasibility", "barrier", "barriers", "challenge", "challenges",
             "implementation", "adoption")):
        detected_query_type = "qualitative"
    elif any(term in normalized_query for term in ("evidence", "effectiveness", "efficacy", "outcome", "trial", "randomized")):
    ...
    expanded_terms.extend(intent_terms)
